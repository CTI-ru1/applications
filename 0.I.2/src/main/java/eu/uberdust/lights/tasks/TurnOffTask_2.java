...
public class TurnOffTask_2 extends TimerTask {
    ...
    public final void run() {
        ...
        if (node.equals("brown")) {
            isScreenLocked = LightController.getInstance().isBrownLocked();
        } else if (node.equals("amber")) {
            isScreenLocked = LightController.getInstance().isAmberLocked();
        } else if (node.equals("moin")) {
            isScreenLocked = LightController.getInstance().isMoinLocked();
        }
